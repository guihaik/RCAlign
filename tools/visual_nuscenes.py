...
class NuScenesExplorer:
    ...
    def render_sample(self,
        ...
        # cols = 2
        cols = 3
        fig, axes = plt.subplots(int(np.ceil(n / cols)), cols, figsize=(16, 6)) #todo
        # fig, axes = plt.subplots(1, 1, figsize=(16, 24))
        ...
        for (_, sd_token), ax in zip(camera_data.items(), axes.flatten()[num_radar_plots + num_lidar_plots:]):
            if show_lidarseg or show_panoptic:
                ...
                # for save single pic
                # self.render_sample_data(sd_token, box_vis_level=box_vis_level, ax=None, out_path=out_path, nsweeps=nsweeps,
                #                         show_lidarseg=False, verbose=False)

        # Change plot settings and write to disk.
        axes.flatten()[-1].axis('off')
        plt.tight_layout()
        fig.subplots_adjust(wspace=0, hspace=0) #todo
    ...
    def render_sample_data(self,
        ...
        if out_path is not None:
            out_path = out_path[:-9]+sd_record['channel']+out_path[-9:]
            plt.savefig(out_path, bbox_inches='tight', pad_inches=0, dpi=200)
