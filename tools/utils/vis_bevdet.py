...
def img_layout(imgs, canvas, canva_size, scale_factor,lidar='mid'):
    if lidar=='mid':
        ...
        img = np.zeros((900 * 2, 1600 * 3 + canva_size, 3),
                       dtype=np.uint8)
        ...
        # canvas = cv2.resize(canvas, (canva_size, int(900 / scale_factor * 2)))
        w_begin = int((1600 * 3))
        img[:, w_begin:, :] = canvas
    return img
...
def parse_args():
    ...
    parser.add_argument(
        '--canva-size', type=int, default=1800, help='Size of canva in pixel') #1000 for mid, 450 for right
    parser.add_argument(
        '--vis-frames',
        type=int,
        default=500,
...
def main():
    ...
    info_path = \
        args.root_path + '/nuscenes2d_temporal_infos_val.pkl'
    ...
    if args.format == 'video':
        ...
        vout = cv2.VideoWriter(
            ...
            args.fps, ((int(1600 * 3 + canva_size))//scale_factor,          # int((1600 * 3 + canva_size)// scale_factor), int(900 * 2 // scale_factor)
    ...
    for cnt, infos in enumerate(
        ...
        for view in views:
            ...
            for aid in range(valid.shape[0]):
                if scores[aid] < args.vis_thred or not pred_flag[aid]:
                    continue
        ...
        img = img_layout(imgs, canvas, canva_size, scale_factor, lidar='right')

        if args.format == 'image':
            cv2.imwrite(os.path.join(vis_dir, '%s.jpg' % infos['token']), img)
        elif args.format == 'video':
            img = cv2.resize(img, (int((1600 * 3 + canva_size)// scale_factor), int(900 * 2 // scale_factor)))
            vout.write(img)
    if args.format == 'video':
        vout.release()
