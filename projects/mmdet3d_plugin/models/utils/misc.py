...
def apply_radar_center_offset(locations, center_offset):
    """
    :param locations:  (1, H, W, 3)
    :param pred_ltrb:  (N, H, W, 4)
    """
    centers_3d = torch.zeros_like(center_offset)
    locations = inverse_sigmoid(locations)
    centers_3d[..., 0] = locations[..., 0] + center_offset[..., 0]  # x1
    centers_3d[..., 1] = locations[..., 1] + center_offset[..., 1]  # y1
    centers_3d[..., 2] = locations[..., 2] + center_offset[..., 2]  #z1
    centers_3d = centers_3d.sigmoid()
...
def gaussian_radius(det_size, min_overlap=0.5):
    ...
    radius = torch.cat([r1.unsqueeze(1), r2.unsqueeze(1), r3.unsqueeze(1)], dim=-1)
    radius = torch.ceil(torch.min(radius, dim=-1)[0])
