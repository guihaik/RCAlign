...
class DeepDeformableFeatureAggregationCuda(BaseModule):
    def __init__(self,
                 use_lidar=False,
                 use_radar=True,
                 use_cam=True,
                 ...
                 norn_cfg=None,
                 init_cfg=None):
        super(DeepDeformableFeatureAggregationCuda, self).__init__()
        pass
...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(self, memory, tgt, query_pos, pos_embed, attn_masks, temp_memory=None, temp_pos=None, radar_memory=None, radar_pos=None, mask=None, reg_branch=None):
        ...
        if radar_memory!=None:
            radar_memory = radar_memory.transpose(0,1).contiguous()
            radar_pos = radar_pos.transpose(0,1).contiguous()
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                if radar_memory!=None:
                    key = torch.cat([key, radar_memory], dim=0)
                    value = torch.cat([value, radar_memory], dim=0)
                    key_pos = torch.cat([key_pos, radar_pos], dim=0)
                query = self.attentions[attn_index](
                    query,
                    key,
                    value,
