...
class StreamPETRHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.radar_embed = nn.Sequential(
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        r_B, r_C, r_H, r_W = radar_feats.shape
        r_num_tokens = r_H * r_W
        radar_memory = radar_feats.permute(0, 2, 3, 1).reshape(r_B, r_num_tokens, r_C)

        pos_embed, cone = self.position_embeding(data, memory_center, topk_indexes, img_metas)

        memory = self.memory_embed(memory)

        #radar feature embedding and positional embeding
        radar_memory = self.radar_embed(radar_memory)
        # radar_pos = self.radar_embedding(pos2posemb3d(radar_memory[:, :, :3]))
        radar_pos = self.radar_embedding(pos2posemb3d(self.coords_bev(radar_feats).to(radar_feats.device)))
        radar_pos = radar_pos.unsqueeze(0).repeat(r_B, 1, 1)
        ...
        outs_dec, _ = self.transformer(memory, tgt, query_pos, pos_embed, attn_mask, temp_memory, temp_pos, radar_memory, radar_pos)
