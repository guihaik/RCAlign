...
from projects.mmdet3d_plugin.models.utils.fps import get_fps
import copy
...
class SparseHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.num_query = num_query + num_radar
        self.num_radar = num_radar
        self.sample_radar = sample_radar
        ...
        self.loss_align = self.constractive_loss
    ...
    def _init_layers(self):
        ...
        self.radar_embedding = nn.Sequential(
            nn.Linear(self.in_channels // 4, self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims),
        )

        self.radar_pos_embedding = nn.Sequential(
            nn.Linear(self.embed_dims * 3 // 2, self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims),
        )
    ...
    def radar_feat_prepare(self, radar_feats, radar_pos_embedding):
        b = radar_feats[0].shape[0]
        feat_flatten = []
        lvl_pos_embedding_flatten = []
        spatial_shapes = []

        for lvl, (feat, pos_embedding) in enumerate(zip(radar_feats, radar_pos_embedding)):
            bs, c, h, w = feat.shape
            spatial_shape = (h, w)
            feat = self.radar_embedding(feat.reshape(bs, c, h*w).transpose(1, 2))
            pos_embedding = pos_embedding.unsqueeze(0).repeat(b, 1, 1)
            spatial_shapes.append(spatial_shape)
            feat_flatten.append(feat)
            lvl_pos_embedding_flatten.append(pos_embedding)
        feat_flatten = torch.cat(feat_flatten, 1)
        lvl_pos_embedding_flatten = torch.cat(lvl_pos_embedding_flatten, 1)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=feat_flatten.device)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        return feat_flatten, lvl_pos_embedding_flatten, spatial_shapes, level_start_index

    def coords_bev(self, pos):
        b, c, h, w = pos.shape
        x_size, y_size, z_size = (h, w, 1)
        ...
        coord_base = torch.cat([batch_x[None], batch_y[None], batch_z[None]], dim=0)
        coord_base = coord_base.view(3, -1).transpose(1, 0)  # (H*W, 3)
        return coord_base

    def radar_position_embedding(self, radar_feats):
        radar_poses = []
        for radar_feat in radar_feats:
            radar_pos = self.radar_pos_embedding(pos2posemb3d(self.coords_bev(radar_feat).to(radar_feat.device)))
            radar_poses.append(radar_pos)
        return radar_poses

    def radar_reference_points(self, radar_feats, topk_indexes, location):
        B, c, h, w = radar_feats[0].shape

        sample_radar_indexes = topk_indexes[:, :self.num_radar, 0]
        new_radar_points = location[torch.arange(B).unsqueeze(1), sample_radar_indexes]

        return new_radar_points

    def forward(self, img_metas, topk_indexes, location, **data):
        """Forward function.
        Args:
        ...
        radar_feats = [data['radar_feats']]
        radar_reference_points = self.radar_reference_points(radar_feats, topk_indexes, location)
        radar_pos = self.radar_position_embedding(radar_feats)

        radar_feat_flatten, radar_lvl_pos_embedding_flatten, radar_spatial_shapes, radar_level_start_index = self.radar_feat_prepare(radar_feats, radar_pos)
        ...
        if self.num_radar!=0:
            reference_points[:, -self.num_radar:] = radar_reference_points
        ...
        outs_dec, align = self.transformer(tgt, query_pos, feat_flatten, spatial_flatten, level_start_index, temp_memory,
                                    temp_pos, attn_mask, reference_points, self.pc_range, data, img_metas,
                                    radar_feat_flatten, radar_lvl_pos_embedding_flatten, radar_spatial_shapes, radar_level_start_index)

        outs_dec = torch.nan_to_num(outs_dec)
        align = torch.nan_to_num(align)
        outputs_classes = []
        outputs_coords = []
        ...
        if mask_dict and mask_dict['pad_size'] > 0:
            ...
            outputs_align = align[:, :, :, mask_dict['pad_size']:, :]
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox = multi_apply(
            ...
            all_gt_bboxes_ignore_list)
        losses_align = []
        for align_idx in range(num_dec_layers):
            img_output = preds_dicts['align'][align_idx][0]
            rad_output = preds_dicts['align'][align_idx][1]
            loss_align = self.loss_align(img_output, rad_output)
        ...
        loss_dict['loss_align'] = losses_align[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_align_i in zip(losses_cls[:-1],
                                                           losses_bbox[:-1],
                                                           losses_align[:-1]):
    ...
    def constractive_loss(self, img, radar):
        img = img / img.norm(p=2, dim=-1, keepdim=True)
        radar = radar / radar.norm(p=2, dim=-1, keepdim=True)
        logit_scale = self.logit_scale.exp()

        logits_per_img = torch.matmul(img, radar.permute(0, 2, 1)) * logit_scale
        logits_per_rad = logits_per_img.permute(0, 2, 1)

        img_labels = torch.stack([torch.arange(len(logits_per_img[0]), device=logits_per_img.device) for i in range(len(logits_per_img))])
        rad_labels = torch.stack([torch.arange(len(logits_per_rad[0]), device=logits_per_rad.device) for i in range(len(logits_per_rad))])

        img_loss = nn.functional.cross_entropy(logits_per_img, img_labels)
        rad_loss = nn.functional.cross_entropy(logits_per_rad, rad_labels)

        loss = (img_loss+rad_loss)/2 * self.cl_weight
        return loss
