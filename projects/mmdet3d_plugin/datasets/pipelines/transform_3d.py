...
class GlobalRotScaleTransImage():
    ...
    def __call__(self, results):
        ...
        results['radar'].rotate(rot_angle)

        # random scale
        self._scale_xyz(results, scale_ratio)
        results["gt_bboxes_3d"].scale(scale_ratio)
        results['radar'].scale(scale_ratio)

        #random translate
        self._trans_xyz(results, trans)
        results["gt_bboxes_3d"].translate(trans)
        results['radar'].translate(trans)

        # from tools.utils.radar_project import RadarProject
        # import cv2
        # radar_proj = RadarProject('./data/train_sample.pkl')
        # for i, img in enumerate(results['img']):
        #     img1 = radar_proj.draw_lidar_bbox3d_on_img(results['gt_bboxes_3d'], img, results['lidar2img'][i],
        #                                                results['img_fields'])
        #     cv2.imwrite(f'img{i}_bbox_crop.png', img1)
